namespace Game.CMF {
	...
	public class WalkerController : Controller {
		...
		[SerializeField] protected CeilingDetector ceilingDetector;

		public Transform MoverTransform => Mover.transform;
		...
		void ControllerUpdate() {
			...
			if (useLocalMomentum)
				_worldMomentum = MoverTransform.localToWorldMatrix * momentum;
		...
		protected virtual Vector3 CalculateMovementDirection() {
			...
			if (cameraTransform == null) {
				_velocity += MoverTransform.right * characterInput.GetHorizontalMovementInput();
				_velocity += MoverTransform.forward * characterInput.GetVerticalMovementInput();
			} else {
				//If a camera transform has been assigned, use the assigned transform's axes for movement direction;
				//Project movement direction so movement stays parallel to the ground;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.right, MoverTransform.up).normalized * characterInput.GetHorizontalMovementInput();
				_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, MoverTransform.up).normalized * characterInput.GetVerticalMovementInput();
			}
		...
		ControllerState DetermineControllerState() {
			...
			bool _isRising = IsRisingOrFalling() && (VectorMath.GetDotProduct(GetMomentum(), MoverTransform.up) > 0f);
		...
		void HandleMomentum() {
			...
			if (useLocalMomentum)
				momentum = MoverTransform.localToWorldMatrix * momentum;
			...
			if (momentum != Vector3.zero) {
				_verticalMomentum = VectorMath.ExtractDotVector(momentum, MoverTransform.up);
				_horizontalMomentum = momentum - _verticalMomentum;
			}

			//Add gravity to vertical momentum;
			_verticalMomentum -= MoverTransform.up * gravity * Time.deltaTime;

			//Remove any downward force if the controller is grounded;
			if (currentControllerState == ControllerState.Grounded && VectorMath.GetDotProduct(_verticalMomentum, MoverTransform.up) < 0f)
			...
			if (currentControllerState == ControllerState.Sliding) {
				...
				Vector3 _pointDownVector = Vector3.ProjectOnPlane(Mover.GetGroundNormal(), MoverTransform.up).normalized;
			...
			if (currentControllerState == ControllerState.Sliding) {
				...
				if (VectorMath.GetDotProduct(momentum, MoverTransform.up) > 0f)
					momentum = VectorMath.RemoveDotVector(momentum, MoverTransform.up);

				//Apply additional slide gravity;
				Vector3 _slideDirection = Vector3.ProjectOnPlane(-MoverTransform.up, Mover.GetGroundNormal()).normalized;
			...
			if (currentControllerState == ControllerState.Jumping) {
				momentum = VectorMath.RemoveDotVector(momentum, MoverTransform.up);
				momentum += MoverTransform.up * jumpSpeed;
			}

			if (useLocalMomentum)
				momentum = MoverTransform.worldToLocalMatrix * momentum;
		}
		...
		void OnGroundContactRegained() {
			...
			if (OnLand != null) {
				...
				//If local momentum is used, transform momentum into world coordinates first;
				if (useLocalMomentum)
					_collisionVelocity = MoverTransform.localToWorldMatrix * _collisionVelocity;
		...
		private bool IsRisingOrFalling() {
			...
			Vector3 _verticalMomentum = VectorMath.ExtractDotVector(GetMomentum(), MoverTransform.up);
		...
		private bool IsGroundTooSteep() {
			...
			return (Vector3.Angle(Mover.GetGroundNormal(), MoverTransform.up) > slopeLimit);
		}
		...
		public void SetMomentum(Vector3 _newMomentum) {
			if (useLocalMomentum)
				momentum = MoverTransform.worldToLocalMatrix * _newMomentum;
			else
